import { useDynamicListVirtualizer } from '@/hooks/useDynamicListVirtualizer'
...
function VirtualListHarness({ totalItems = 100, firstRowHeight = 40 }: { totalItems?: number; firstRowHeight?: number }) {
  const { virtualItems, totalHeight, listContainerRef, measureElement } = useDynamicListVirtualizer(totalItems, 40)
    ...
    <div data-cy="scroll-root" className="relative h-[200px] overflow-y-auto">
      <div ref={listContainerRef} data-cy="virtual-list" className="relative w-full" style={{ height: `${totalHeight}px` }}>
        {virtualItems.map(({ index, start }) => (
          <VirtualRow key={index} index={index} start={start} height={index === 0 ? firstRowHeight : 40} measureElement={measureElement} />
        ))}
      </div>
    </div>
  )
...
describe('useDynamicListVirtualizer', () => {
