...
/** Max HH:MM:SS shown in the sleep timer panel (keeps layout stable when time is adjusted up). */
...
export interface SleepTimerPanelProps {
  /** Clears the custom-time draft whenever the surface reappears */
  isVisible: boolean
  timerSet: boolean
...
export default function SleepTimerPanel({
  ...
  // A half-typed custom time from last time the panel was open is never what the user wants now
  useEffect(() => {
    if (isVisible) setCustomTime('')
  }, [isVisible])
  ...
  return (
    <div className={mergeClasses('w-full overflow-y-auto px-1 py-0.5', className)}>
      {timerSet ? (
        <>
        ...
        <>
          ...
          <form className="flex items-center justify-center gap-2 px-4 py-3" onSubmit={submitCustomTime}>
            <TextInput value={customTime} type="number" step="any" min={0.1} placeholder={t('LabelTimeInMinutes')} onChange={setCustomTime} className="w-48" />
