...
interface LibraryFilesAccordionProps {
...
export default function LibraryFilesAccordion({
  libraryItem,
  user,
  keepOpen = false,
  inModal = false,
  ...
  const headerActions = useMemo(
    () =>
      userIsAdmin ? (
        <HeaderActionButton color={showFullPath ? 'bg-button-selected-bg' : ''} onClick={toggleFullPath}>
          {t('ButtonFullPath')}
        </HeaderActionButton>
      ) : null,
  ...
  return (
    <>
      <AccordionSection
        ...
        className={className}
      >
        <Table headers={tableHeaders} containerRef={tableContainerRef} title={t('HeaderLibraryFiles')}>
