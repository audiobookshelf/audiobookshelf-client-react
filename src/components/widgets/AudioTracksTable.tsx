...
/**
 * Collapsible table displaying audio tracks for books.
 *
 * Features:
 * - Track number, filename, codec, bitrate, size, duration columns
 * - Responsive column hiding (codec/bitrate hidden on smaller screens)
 * - Toggle full path button (admin only)
 * - Context menu with download, delete, and more info actions
 */
export default function AudioTracksTable({ libraryItem, user, keepOpen = false, expanded: expandedProp = false, className }: AudioTracksTableProps) {
  ...
  const headerActions = useMemo(() => {
    ...
    const manageTracksBtn = user.permissions?.update ? (
      <Link key="manage-tracks" href={`/library/${libraryItem.libraryId}/item/${libraryItem.id}/tracks`} className="me-2" onClick={(e) => e.stopPropagation()}>
        <Btn size="small">{t('ButtonManage')}</Btn>
      </Link>
    ) : null
